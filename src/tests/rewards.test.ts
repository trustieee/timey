...
describe('Rewards Functionality', () => {
    ...
    test('Should use a reward and apply it permanently', async () => {
        ...
        const furtherUpdatedProfile = await playerProfile.useReward(
            updatedProfile,
            RewardType.EXTEND_PLAY_TIME,
            60
        );
    ...
    test('Should not allow using rewards if none available', async () => {
    ...
    test('Should award rewards when leveling up', async () => {
        ...
        const addXpResult = await playerProfile.addXp(profile, APP_CONFIG.PROFILE.XP_PER_LEVEL[0]);
        ...
        const secondLevelUpResult = await playerProfile.addXp(addXpResult, APP_CONFIG.PROFILE.XP_PER_LEVEL[1]);
    ...
    test('Should retrieve permanent bonus values correctly', async () => {
        ...
        profile = await playerProfile.useReward(profile, RewardType.EXTEND_PLAY_TIME, 60);
        profile = await playerProfile.useReward(profile, RewardType.EXTEND_PLAY_TIME, 60);

        // Add permanent cooldown reduction
        profile = await playerProfile.useReward(profile, RewardType.REDUCE_COOLDOWN, 30);
    ...
    test('Should handle using multiple rewards of different types correctly', async () => {
        ...
        updatedProfile = await playerProfile.useReward(updatedProfile, RewardType.EXTEND_PLAY_TIME, 10);
        updatedProfile = await playerProfile.useReward(updatedProfile, RewardType.EXTEND_PLAY_TIME, 15);

        // Then, add cooldown reductions
        updatedProfile = await playerProfile.useReward(updatedProfile, RewardType.REDUCE_COOLDOWN, 5);
        updatedProfile = await playerProfile.useReward(updatedProfile, RewardType.REDUCE_COOLDOWN, 10);
    ...
    test('Should handle extremely large reward values', async () => {
        // Start with 1 reward
    ...
    test('Should work with zero and negative reward values', async () => {
        // Using zero as reward value
        testProfile.rewards.available = 2;
        let updatedProfile = await playerProfile.useReward(
            testProfile,
            RewardType.EXTEND_PLAY_TIME,
            0
        );
        ...
        updatedProfile = await playerProfile.useReward(
            updatedProfile,
            RewardType.EXTEND_PLAY_TIME,
            -10
        );
    ...
    test('Should persist permanent rewards correctly through profile save/load cycle', async () => {
        // Set up a profile with rewards
        testProfile.rewards.available = 2;
        const profile = await playerProfile.useReward(
            testProfile,
            RewardType.EXTEND_PLAY_TIME,
            45
        );
    ...
    test('Should handle multiple reward applications to reach max values', async () => {
        ...
        for (let i = 0; i < 10; i++) {
            profile = await playerProfile.useReward(profile, RewardType.REDUCE_COOLDOWN, 1);
        }
    ...
    test('Should add exactly the specified reward amounts', async () => {
        ...
        profile = await playerProfile.useReward(profile, RewardType.EXTEND_PLAY_TIME, 5);
        profile = await playerProfile.useReward(profile, RewardType.EXTEND_PLAY_TIME, 7);
        profile = await playerProfile.useReward(profile, RewardType.EXTEND_PLAY_TIME, 13);

        // Cooldown rewards with specific values
        profile = await playerProfile.useReward(profile, RewardType.REDUCE_COOLDOWN, 4);
        profile = await playerProfile.useReward(profile, RewardType.REDUCE_COOLDOWN, 9);
