...
describe('Timer Functionality and Day Transitions', () => {
  ...
  describe('Day Initialization and Transitions', () => {
    test('Should create a new day with default chores', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();

      // Initialize today
      const updatedProfile = await initializeDay(profile);
    ...
    test('Should finalize previous day with penalties when creating a new day', async () => {
      const profile = createTestProfile();
    ...
    test('Should check and finalize multiple previous days', async () => {
      const profile = createTestProfile();
      ...
      const updatedProfile = await checkAndFinalizePreviousDays(profile);
  ...
  describe('Chore Status Updates', () => {
    test('Should update chore status and add XP when completing a chore', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();
      ...
      const updatedProfile = await updateChoreStatus(profile, 0, 'completed');
    ...
    test('Should remove XP when un-completing a chore', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();
      ...
      let updatedProfile = await updateChoreStatus(profile, 0, 'completed');

      // Verify initial XP gain
      expect(updatedProfile.history[today].xp.gained).toBe(APP_CONFIG.PROFILE.XP_FOR_CHORE);

      // Now un-complete the chore
      updatedProfile = await updateChoreStatus(updatedProfile, 0, 'incomplete');
    ...
    test('Should handle N/A chore status correctly', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();
      ...
      let updatedProfile = await updateChoreStatus(profile, 0, 'completed');
      updatedProfile = await updateChoreStatus(updatedProfile, 0, 'na');
    ...
    test('Should track XP correctly with mixed chore statuses', async () => {
      ...
      // Complete 5 chores
      for (let i = 0; i < 5; i++) {
        profile = await updateChoreStatus(profile, i, 'completed');
      }

      // Mark 3 chores as N/A
      for (let i = 5; i < 8; i++) {
        profile = await updateChoreStatus(profile, i, 'na');
      }
      ...
      const finalizedProfile = await finalizeDayProgress(profile, today);
  ...
  describe('Timer-Based Reward System', () => {
    test('Should correctly apply play time extension reward', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();
    ...
    test('Should correctly apply cooldown reduction reward', async () => {
      const profile = createTestProfile();
      const today = getLocalDateString();
