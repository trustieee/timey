...
const DayHistory: React.FC<DayHistoryProps> = ({
  ...
  const ChoreStatusIndicator = ({ status }: { status: ChoreStatus }) => {
    ...
    return (
      <div className={`${config.bgColor} ${config.textColor} px-3 py-1 rounded-full flex items-center gap-1 ${config.hoverColor} cursor-pointer transition-all shadow-md text-xs font-medium`}>
  ...
  const progressPercentage = choreStats.total > 0 
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div
        className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-lg shadow-xl overflow-hidden border border-slate-700"
      >
        <div
          className={`p-4 transition-colors duration-200 ${isDayExpanded ? "bg-opacity-80" : "hover:bg-slate-800"}`}
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 relative">
            <div className="flex items-center space-x-2">
              <div className="text-indigo-400 w-5 h-5 flex-shrink-0">
                <svg 
                  ...
                  className={`transform transition-transform ${isDayExpanded ? "rotate-90" : ""}`}
                >
            ...
            <div className="flex flex-wrap gap-2">
              <div 
                className="flex items-center bg-indigo-900/30 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-inner text-indigo-300 font-medium hover:bg-indigo-900/40 transition-colors"
              >
                <svg className="w-4 h-4 mr-1 opacity-80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M13.5 3H12H8C6.34315 3 5 4.34315 5 6V18C5 19.6569 6.34315 21 8 21H16C17.6569 21 19 19.6569 19 18V9.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                  <path d="M17 7L13 3M17 7V3H13" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              ...
              {playStats.totalSessions > 0 && (
                <div 
                  className="flex items-center bg-emerald-900/30 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-inner text-emerald-300 font-medium hover:bg-emerald-900/40 transition-colors"
                >
                  <svg className="w-4 h-4 mr-1 opacity-80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div 
                className="flex items-center bg-violet-900/30 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-inner text-violet-300 font-medium hover:bg-violet-900/40 transition-colors"
              >
                <svg className="w-4 h-4 mr-1 opacity-80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {choreStats.total > 0 && (
                <div 
                  className="flex items-center bg-amber-900/30 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-inner text-amber-300 font-medium relative hover:bg-amber-900/40 transition-colors"
                >
                  <svg className="w-4 h-4 mr-1 opacity-80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M9 5H7C5.89543 5 5 5.89543 5 7V19C5 20.1046 5.89543 21 7 21H17C18.1046 21 19 20.1046 19 19V7C19 5.89543 18.1046 5 17 5H15" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                    <path d="M12 12L9 15M12 12L15 15M12 12V3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span>
          ...
          <div className="overflow-hidden">
            <div className="p-4 pt-0">
              <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-5">
                {/* Chores Section */}
                <div 
                  className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700 shadow-lg hover:shadow-xl transition-shadow duration-300"
                >
                  <div className="flex items-center mb-4">
                    <svg className="w-5 h-5 text-amber-400 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M9 6L20 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                      <path d="M3.5 6H6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                      <path d="M3.5 12H6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                      <path d="M3.5 18H6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                      <circle cx="7.5" cy="6" r="1.5" stroke="currentColor" strokeWidth="1.5"/>
                      <circle cx="7.5" cy="12" r="1.5" stroke="currentColor" strokeWidth="1.5"/>
                  ...
                  {dayData.chores && dayData.chores.length > 0 ? (
                    <ul className="space-y-2">
                      {dayData.chores.map((chore) => (
                        <li
                          key={chore.id}
                          className={`flex items-center justify-between p-3 rounded-lg bg-slate-700/50 backdrop-blur-sm border border-slate-600 shadow-sm transition-all duration-200 hover:bg-slate-700/70 ${isUpdating ? "opacity-70" : "opacity-100"}`}
                        >
                          <div className="flex items-start space-x-3">
                            <div onClick={(e) => cycleChoreStatus(e, chore.id)} className="cursor-pointer pt-0.5">
                            ...
                            <span className={`font-medium break-words ${
                              chore.status === "completed" ? "text-slate-300 line-through" : 
                              chore.status === "na" ? "text-slate-500" : "text-white"
                            }`}>
                              {chore.text}
                            </span>
                          </div>
                          
                          <div onClick={(e) => cycleChoreStatus(e, chore.id)} className="cursor-pointer">
                    ...
                    <div className="flex flex-col items-center justify-center py-6 text-center">
                      ...
                      <p className="text-slate-400 text-sm">No chores assigned for this day</p>
                    </div>
                  )}
                </div>
                ...
                >
                  <div className="flex items-center mb-4">
                    <svg className="w-5 h-5 text-emerald-400 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {playStats.totalSessions > 0 && (
                      <span className="ml-auto text-xs font-medium text-slate-400">
                        {playStats.totalSessions} session{playStats.totalSessions !== 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                  
                  {dayData.playTime?.sessions && dayData.playTime.sessions.length > 0 ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-indigo-900/20 backdrop-blur-sm rounded-lg p-3 border border-indigo-900/50">
                          <p className="text-xs text-indigo-300 mb-1">Total Sessions</p>
                          <div className="flex items-baseline">
                            <span className="text-2xl font-bold text-white">{playStats.totalSessions}</span>
                            <span className="text-indigo-400 text-xs ml-1">sessions</span>
                          </div>
                        </div>
                        <div className="bg-emerald-900/20 backdrop-blur-sm rounded-lg p-3 border border-emerald-900/50">
                          <p className="text-xs text-emerald-300 mb-1">Play Time</p>
                          <div className="flex items-baseline">
                            <span className="text-2xl font-bold text-white">{playStats.formattedPlayTime.replace(/hrs?/i, '').replace(/mins?/i, '')}</span>
                            <span className="text-emerald-400 text-xs ml-1">
                              {playStats.formattedPlayTime.includes('hr') ? 'hours' : 'minutes'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-slate-300 mb-2 flex items-center">
                          <svg className="w-4 h-4 mr-1 text-slate-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex flex-col items-center justify-center py-6 text-center">
                      ...
                      <p className="text-slate-400 text-sm">No play activity recorded for this day</p>
                    </div>
                  )}
                </div>
              </div>
